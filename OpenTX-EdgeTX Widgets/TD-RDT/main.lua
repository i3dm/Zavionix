...
local function drawTurbine(Me,x,y)
	if getValue(Me.param.Tmp2) > 100 then
		lcd.drawText(x,y-10,"No Status",MIDSIZE + WHITE)
	else
		lcd.drawText(x,y-10,Me.ECU_Status[getValue(Me.param.Tmp2)],MIDSIZE + WHITE)
	end
	lcd.drawText(x,y+33,"RPM:", WHITE)
	if Me.options.ECU_Type == 7 or Me.options.ECU_Type == 8 then
		lcd.drawText(x+52,y+26,format_thousand(getValue(Me.param.RPM)),MIDSIZE + WHITE)
	else
		lcd.drawText(x+52,y+26,format_thousand(getValue(Me.param.RPM)*100),MIDSIZE + WHITE)
	end
	lcd.drawText(x,y+58,"EGT:", WHITE)
	lcd.drawText(x+52,y+51,getValue(Me.param.Tmp1).."c",MIDSIZE + WHITE)
	lcd.drawText(x,y+83,"Pump:", WHITE)
	if Me.options.ECU_Type < 3 then
		lcd.drawText(x+52,y+76,round(getValue(Me.param.A4)*Me.A4Mult,2).."v",MIDSIZE + WHITE)
	elseif Me.options.ECU_Type == 7 then
		if getValue(Me.param.Tmp2) == 11 then -- is ECU in running state?
			lcd.drawText(x+52,y+76,round(getValue(Me.param.Curr)*100,2),MIDSIZE + WHITE)
		end
	else
		lcd.drawText(x+52,y+76,round(getValue(Me.param.A4)*Me.A4Mult,2),MIDSIZE + WHITE)
	end
	lcd.drawText(x,y+108,"ECU:", WHITE)
	if Me.options.ECU_Type == 8 then
		lcd.drawText(x+52,y+101,round(getValue(Me.param.A3)*0.1,2).."v",MIDSIZE + WHITE)
	else
		lcd.drawText(x+52,y+101,round(getValue(Me.param.A3),2).."v",MIDSIZE + WHITE)
	end
end
...
local function refreshWidget(Me)
	...
	lcd.drawText(363,15,round(getValue(Me.param.RxBt),1).."v",MIDSIZE + WHITE)
	--Tx Battery voltage
	lcd.drawText(422,3,"TxBatt",SMLSIZE+WHITE)
	lcd.drawText(422,15,round(getValue(Me.param.TxV),1).."v",MIDSIZE + WHITE)
	...
	if Me.options.ECU_Type == 5 or Me.options.ECU_Type == 6 or Me.options.ECU_Type == 7  then
		...
		lcd.drawText(168,199,"Current:", SMLSIZE + WHITE)
		lcd.drawText(168,212, round(getValue(Me.param.Curr),1).."A",MIDSIZE + WHITE)
	end
	if Me.options.ECU_Type == 6 then
		--Lintom mAh
		lcd.drawText(227,199,"Batt mAh:", SMLSIZE + WHITE)
		--lcd.drawText(230,212, round(getValue(Me.param.Fuel),1).."mAh",MIDSIZE )
		lcd.drawText(227,212, round(getValue(Me.param.Fuel),1),MIDSIZE + WHITE)
	end
	--Throttle
	lcd.drawText(14,199,"Throttle:", SMLSIZE + WHITE )
	if Me.options.ECU_Type == 7 then
		lcd.drawText(14,212, round((getValue("thr") + 1024)/20.48).."%",MIDSIZE + WHITE)
	else
		if thr_val >= 0 and thr_val <= 100 then
			lcd.drawText(14,212, thr_val.."%",MIDSIZE + WHITE)
		end
	end
	--Fuel
	lcd.drawText(78,199,"Fuel:", SMLSIZE + WHITE)
	--set the text to red if we are below the fuel warning level
    local fuel_color = (model.getGlobalVariable(8, getFlightMode()) == 1) and RED or WHITE
	lcd.drawText(78,212, fuel_val.."ml",MIDSIZE + fuel_color)

    --display the ECU type
	lcd.drawText(212,252,Me.ECU_Name, SMLSIZE + WHITE)
